...
void LCD_Draw_Square_Fill(uint16_t Xpos, uint16_t Ypos, uint16_t height, uint16_t length, uint16_t color) {
    for (int x = Xpos - length/2; x < Xpos + length/2; x++) {
...
static inline uint16_t string_length(const char *str) {
    uint16_t length = 0;
    while (str[length] != '\0') {
        length++;
    }
...
void LCD_DisplayStringCentered(uint16_t y, const char *str) {
    uint16_t x_start;
    uint16_t char_width = 12;
    uint16_t str_length = string_length(str);
    uint16_t total_width = str_length * char_width;

    if (total_width < LCD_PIXEL_WIDTH) {
        x_start = (LCD_PIXEL_WIDTH - total_width) / 2;
    } else {
        x_start = 0; // Text too wide; start at the edge
    }
    // Display each character
    for (uint16_t i = 0; i < str_length; i++) {
        LCD_DisplayChar(x_start + i * char_width, y, str[i]);
    }
}
